...
class MyFatoorahPaymentGatewayService implements PaymentGatewayService
{
    ...
    public function start(Payable $payable, string $paymentMethodId): array
    {
        ...
        //  TODO: This should be removed in favor of actual order amount
        $amount = collect($payable->items())->reduce(
            fn(BigDecimal $carry, $item) =>
            $carry->plus(
                $item->price()->getAmount()->multipliedBy(
                    BigDecimal::of($item->quantity())
                )
            ),
            BigDecimal::zero()
        );
        ...
        $fields = [
            ...
            //  """
            'CustomerMobile' => $user->phone ? Str::replace($countryCode, '', $user->phone->formatE164()) : null,
