...
class GoogleAuthProvider implements AuthProvider
{
    ...
    {
        $extra = encrypt(json_encode(['sessionId' => session()->getId()]));
    ...
    public function callback()
    {
        ...
        try {
            $extra = json_decode(decrypt(request()->state), true);

            $sessionId = $extra['sessionId'] ?? null;
            if (! $sessionId) {
                Log::error('Session ID not found in Google callback request', [
                    'query' => request()->query(),
                ]);
            ...
            // TODO: Migrate session cart to user cart
            // TODO: Migrate session favorites to user favorites
            // NOTE: This better be done in two separate try-catch blocks
        } catch (Exception $e) {
            Log::critical('Error migrating session data after Google login: '.$e->getMessage(), [
                'session_id' => request()->query('session'),
                'user_id' => $user->id ?? null,
            ]);
            // fail silently without disrupting the user's login'
        }
